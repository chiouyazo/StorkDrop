...
public partial class UpdatesViewModel : ObservableObject
{
    ...
    public async Task LoadAsync()
    {
        ...
        {
            ErrorMessage = LocalizationManager.GetString("Error_ServerConnectionFailed") + ": " + ex.Message;
        }
        finally
        {
            IsLoading = false;
    ...
    private async Task UpdateSingleAsync(UpdateItemViewModel update)
    {
        ...
        {
            ErrorMessage =
                LocalizationManager.GetString("Error_UpdateProductFailed").Replace("{0}", update.Title)
                + ": "
                + ex.Message;
        }
    }
