...
public partial class InstalledViewModel : ObservableObject
{
    ...
    {
        bool needsAdmin = ElevationHelper.PathRequiresAdmin(product.InstalledPath)
            && !ElevationHelper.IsRunningAsAdmin();

        string confirmMessage = LocalizationManager.GetString("Confirm_Uninstall")
        ...
        try
        {
            ...
            if (installed is not null)
            {
                ...
                _dialogService.ShowInfo(
                    LocalizationManager.GetString("Info_UninstallSuccess")
                        .Replace("{0}", product.Title));
            }
        }
        catch (FileLockedException ex)
        {
            string message = string.IsNullOrEmpty(ex.ProcessNames)
                ...
                : LocalizationManager.GetString("Error_UninstallFileLocked", ex.FileName, ex.ProcessNames);
            _dialogService.ShowError(message);
        }
        catch (Exception ex)
        {
            _dialogService.ShowError(LocalizationManager.GetString("Error_UninstallFailed") + ": " + ex.Message);
        }
    }
}
