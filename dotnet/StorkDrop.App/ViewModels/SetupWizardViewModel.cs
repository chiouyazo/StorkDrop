    ...
    private async Task TestConnectionAsync()
    {
        try
        {
            ...
            ConnectionTestMessage = IsConnectionValid
                ...
                : LocalizationManager.GetString("Error_ConnectionFailed") + $" (HTTP {(int)response.StatusCode})";
            IsConnectionTested = true;
        }
        catch (Exception ex)
        {
            IsConnectionValid = false;
            ConnectionTestMessage = LocalizationManager.GetString("Error_ConnectionError") + ": " + ex.Message;
            IsConnectionTested = true;
        }
    }
